...
def get_mAP(predictions, classes):
    ...
    for cl_id in range(nb_classes):
        ...
        # sort by confidence
        sorted_ind = np.argsort(-cur_class_probs)
        sorted_scores = np.sort(-cur_class_probs)
        cur_class_bboxes = cur_class_bboxes[sorted_ind, :]
        cur_class_ids = [cur_class_ids[x] for x in sorted_ind]
        ...
        for j, img_id in enumerate(cur_class_ids):
            ...
            cur_gt = np.asarray([np.asarray(c) for c in cur_gt])  # list(np.asarray(cur_class_gt_boxes[j])[cur_gt_ids])
            # cur_gt = np.asarray([tuple(c) for c in cur_gt])
            n_positives += len(cur_gt)

            if len(cur_gt) > 0:
                ixmin = np.maximum(cur_gt[:, 0], cur_bb[0])
                iymin = np.maximum(cur_gt[:, 1], cur_bb[1])
                ixmax = np.minimum(cur_gt[:, 2], cur_bb[2])
                iymax = np.minimum(cur_gt[:, 3], cur_bb[3])
                iw = np.maximum(ixmax - ixmin + 1., 0.)
                ih = np.maximum(iymax - iymin + 1., 0.)
                inters = iw * ih

                # union
                uni = ((cur_bb[2] - cur_bb[0] + 1.) * (cur_bb[3] - cur_bb[1] + 1.) +
                       (cur_gt[:, 2] - cur_gt[:, 0] + 1.) *
                       (cur_gt[:, 3] - cur_gt[:, 1] + 1.) - inters)
