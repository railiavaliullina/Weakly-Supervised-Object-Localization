...
def get_cam_for_image(path, preprocessing, model, fc, activations, save_cam=False, path_to_save=''):
    ...
    softmax_outs, labels_idxs = output.sort(0, True)  # от большего к меньшему
...
def match_gt_and_bbs(gt_boxes, pred_boxes, iou_thr):
    """
    ...
    """
    tp, fp = 0, 0
    pred_boxes_ids = range(len(pred_boxes))
    gt_boxes_ids = range(len(gt_boxes))

    if len(pred_boxes_ids) > 0 and len(gt_boxes_ids) > 0:
        gts_ids, preds_ids, ious = [], [], []
        ...
        for ipb, pred_box in enumerate(pred_boxes):
            for igb, gt_box in enumerate(gt_boxes):
                ...
                if iou > iou_thr:
                    gts_ids.append(igb)
                    preds_ids.append(ipb)
                    ious.append(iou)

        sorted_ious = np.argsort(ious)[::1]
        if len(sorted_ious) > 0:
            matched_gts_ids, matched_preds_ids = [], []
            for idx in sorted_ious:
                gt_id = gts_ids[idx]
                pr_id = preds_ids[idx]

                if gt_id not in matched_gts_ids and pr_id not in matched_preds_ids:
                    matched_gts_ids.append(gt_id)
                    matched_preds_ids.append(pr_id)
            tp = len(matched_gts_ids)
            fp = len(pred_boxes) - len(matched_preds_ids)
    return tp, fp
...
def get_mAP(predictions, classes):
    ...
    for i in range(nb_iters):
        ...
        for cl_id in range(nb_classes):
            # for l in range(len(predicted_boxes[cl_id])):
            # predictions_list.append((img_filename, predicted_boxes[cl_id][l], classes_ids[cl_id], probs[cl_id][l], gt_boxes, labels))
            predictions_list.append((img_filename, predicted_boxes[cl_id], classes_ids[cl_id], probs[cl_id], gt_boxes, labels))
        gt_list.append((img_filename, gt_boxes, labels))
    ...
    for cl_id in range(nb_classes):
        ...
        for j, img_id in enumerate(cur_class_ids):
            cur_prob = np.asarray(cur_class_probs[j])
            sorted_by_prob_ind = np.argsort(-cur_prob)
            cur_bb = np.asarray(cur_class_bboxes[j])[sorted_by_prob_ind, :]
            cur_gt = cur_class_gt_boxes[j]
            cur_gt = np.asarray([np.asarray(c) for c in cur_gt])
            n_positives += len(cur_gt)

            tp[j], fp[j] = match_gt_and_bbs(cur_gt, cur_bb, 0.5)
        ...
        ap_values.append(ap)
        print(cl_id, class_str + f' AP: {ap * 100} %, mAP: {np.mean(ap_values)  * 100} %')
...
if __name__ == '__main__':
    ...
    # mAP: 0.7909829249092112 %
    # 0.98
    # 1.04
    # mAP: 1.0469120839142396 %
    # 1.11 predictions_3_last
    # mAP: 1.1787063423981907 % predictions_3_last_
    # mAP: 1.187390730536975 % predictions_3_last_1 60, 80
    # 1.16
